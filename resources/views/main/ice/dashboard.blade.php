<x-app-layout>
    ...
    <div x-data="{ tab: 'companies', openCompanyModal: false, openJobModal: false }" class="flex h-full min-h-screen bg-gray-50">
        ...
        <div class="w-40 border-r border-gray-200 p-4 flex flex-col space-y-2 bg-white">
            ...
            <button @click="tab = 'jobs'" :class="tab === 'jobs' ? 'font-bold text-orange-600' : 'text-gray-700'"
                class="block w-full text-left px-2 py-1 rounded hover:bg-gray-100 transition">
                Jobs
            </button>
            <button @click="tab = 'surveys'" :class="tab === 'surveys' ? 'font-bold text-orange-600' : 'text-gray-700'"
                class="block w-full text-left px-2 py-1 rounded hover:bg-gray-100 transition">
                Surveys
            </button>
        </div>
        ...
        <div class="flex-1 p-6 overflow-y-auto">
            ...
            <div x-show="tab === 'companies'" x-transition>
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="openCompanyModal = true"
            ...
            <div x-show="tab === 'jobs'" x-transition>
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="openJobModal = true"
                        class="rounded bg-orange-500 px-4 py-2 text-white hover:bg-orange-700 transition">
                        + Create Job
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($jobs ?? [] as $job)
                        <a href="{{ route('jobs.show', $job) }}"
                            class="block rounded-lg bg-white p-5 shadow hover:shadow-lg transition">
                            <h4 class="font-semibold text-gray-800">{{ $job->title }}</h4>
                            <p class="text-sm text-gray-500">{{ $job->company->name ?? 'No company' }}</p>
                            <p class="text-sm text-gray-600 line-clamp-3">{{ $job->description ?? 'No description' }}
                            </p>
                        </a>
                    @empty
        ...
        <div>
            ...
            <div x-show="openCompanyModal" x-cloak
                ...
                <div @click.away="openCompanyModal = false"
                    class="bg-white rounded-lg w-full max-w-lg p-6 overflow-y-auto max-h-[90vh]">
